...
class LeoApp:
    ...
    def checkForOpenFile(self, c: Cmdr, fn: str) -> None:
        ...
        if d is None or g.unitTesting or g.app.batchMode or g.app.reverting or g.app.inBridge:
            return
...
class LoadManager:
    ...
    def computeLoadDir(self) -> str:
        ...
        try:
            if path := g.__file__:
                ...
                loadDir = None
            if not loadDir or not g.os_path_exists(loadDir) or not g.os_path_isdir(loadDir):
                loadDir = os.getcwd()
    ...
    def scanOptions(self, fileName: str, pymacs: bool) -> dict[str, Any]:
        ...
        options:
          ...
          --no-black-sentinels  disable black sentinels
        ...
        try:
            if not g.isValidPython:
                ...
                try:
                    ...
                    d = g.EmergencyDialog(title='Python Version Error', message=message)
                    d.run()
                except Exception:
                    g.es_exception()
        ...
        for name in rf.getRecentFiles()[:n]:
            ...
            if groupedEntries:
                ...
                if len(entry['dirs']) > 1 or rf_always:  # sub menus
                    ...
                    c.add_command(
                        menu.getMenu(baseName),
                        label=dirName,
                        command=recentFilesCallback,
                        underline=0,
                    )
