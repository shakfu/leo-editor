...
class AutoCompleterClass:
    ...
    def autoComplete(self, event: LeoKeyEvent | None = None) -> None:
        ...
        c, k = self.c, self.k
        w = event.w if event else c.get_focus()
        ...
        if self.use_jedi:
            ...
            if jedi:
                aList = (
                    ...
                    self.get_jedi_completions(prefix) or self.get_leo_completions(prefix)
                )
                d[prefix] = aList
                return aList
        ...
        if state == 0:
            ...
            if directory:
                ...
                if not g.os_path_exists(directory):
                    g.es_print(
                        '@string initial-chooser-directory not found',
...
class GetArg:
    ...
    def do_tab_cycling(self, common_prefix: str, tabList: list[str]) -> None:
        ...
        # #323.
            ...
            self.cycling_prefix
            and s.startswith(self.cycling_prefix)
            and sorted(self.cycling_tabList) == sorted(tabList)  # Bug fix: 2016/10/14
        ):
    ...
    def should_end(self, char: str, stroke: Stroke) -> bool:
        ...
        return (
            ...
            or char == '\t'
            and char in k.getArgEscapes  # The Find Easter Egg.
        )
...
class KeyHandlerClass:
    ...
    '''
        ...
        for prefix in (
            ...
            'Alt+Ctrl',
            'Alt+Shift',
            'Alt',  # 'Alt+Key': done by Alt.
            'Ctrl+Meta+Shift',
            'Ctrl+Meta',
            'Ctrl+Shift',
            'Ctrl',  # Ctrl+Key: done by Ctrl.
            'Meta+Key',
            'Meta+Shift',
            'Meta',
            'Shift',
    ...
    def printButtons(self, event: LeoKeyEvent | None = None) -> None:
        ...
        for aList2 in [
            g.app.config.atLocalButtonsList,
            g.app.config.atLocalCommandsList,
        ]:
            for p in aList2:
                data.append(
                    (p.h, 'L'),
    ...
    def doMode(self, event: LeoKeyEvent) -> bool:
        ...
        if handler is not None:
            handler(event)
        if trace:
            handler_name = handler is not None and handler.__name__ or '<no handler>'
    ...
    def doBinding(self, event: LeoKeyEvent) -> bool:
        ...
        if bi and bi.commandName in k.killedBindings:
            if trace:
                g.trace(f"{stroke!s} {bi.commandName}: in killed bindings")
            return False
        # Execute the command if the binding exists.
        if bi:
            # A superb trace. !s gives shorter trace.
            if trace:
                g.trace(f"{stroke!s} {bi.commandName}")
            c.doCommandByName(bi.commandName, event)
            return True
        # No binding exists.
        if trace:
            g.trace(f"{stroke!s}: no binding")
        return False
    ...
    def getPaneBinding(self, event: LeoKeyEvent) -> g.BindingInfo | None:
        ...
        if k.isPlainKey(stroke) and w == c.frame.body.widget and state in ('insert', 'overwrite'):
            return None
        for key, name in (
    ...
    def modeHelpHelper(self, d: g.SettingsDict | None) -> None:
        c, k = self.c, self
        tabName = 'Mode'
        c.frame.log.clearTab(tabName)
        data, n = [], 0
...
class ModeInfo:
    ...
    def createModeBindings(self, w: QTextMixin | None) -> None:
