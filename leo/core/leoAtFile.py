...
class AtFile:
    ...
    def findFilesToRead(self, root: Position, all: bool) -> list[Position]:  # pragma: no cover
        ...
        while p and p != after:
            ...
            if not p.h.startswith('@'):
                ...
                or p.isAtAutoNode()
                or p.isAtEditNode()
                or p.isAtShadowFileNode()
                or p.isAtFileNode()
                or p.isAtCleanNode()
                or p.isAtJupytextNode()
            ):
                files.append(p.copy())
                p.moveToNodeAfterTree()
    ...
    def replaceFile(
        ...
        unchanged = (
            ...
            or ignoreBlankLines
            and at.compareIgnoringBlankLines(old_contents, contents)
        )
        ...
        if at.explicitLineEnding:  # pragma: no cover
            ok = (
                ...
                or ignoreBlankLines
                and at.compareIgnoringLineEndings(old_contents, contents)
            )
            if not ok:
...
class FastAtRead:
    ...
    def scan_lines(
        ...
        # at+others, at+<section>, and at+all update the stack.
        stack: list[tuple[str, int, list[str]]] = []  # Entries are (gnx, indent, body)
        ...
        for i, line in enumerate(lines[start:]):
            ...
            if m := self.comment_pat.match(line):
                ...
                if delim1:
                    ...
                    comment_delim1, comment_delim2 = (
                        delim2,
                        delim3,
                    )  # pragma: no cover (unimportant)
...
# @+node:ekr.20240410111658.3: ** class LeoIOStatus
class LeoIOStatus:
