...
class GoToCommands:
    ...
    def find_file_line_helper(
        ...
        valid = (
            root.isAtCleanNode()
            or root.isAtEditNode()
            or root.isAtFileNode()
    ...
    def find_node_start(self, p: Position, s: str | None = None) -> int | None:
        ...
        node_pat = re.compile(rf"\s*{re.escape(delim1)} ?@\+node:{re.escape(p.gnx)}:")
        ...
        if root.h.endswith('.vue'):
            node_pat2 = re.compile(rf"\s{re.escape('//')} ?@\+node:{re.escape(p.gnx)}:")
            for i, s in enumerate(contents):
